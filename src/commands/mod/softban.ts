...
function execute(intr: ChatInputCommandInteraction<"cached">, logger: Logger) {
	...
	if (
		...
		intr.editReply(e`{cross} I don't have permissions to softban users`);
	...
	if (targetMember && !targetMember.bannable) {
		intr.editReply(e`{cross} I cannot softban the user to target`);
	...
	const query = `${oneLine(e`
			...
			softban ${bold(target.tag)} (${target.id})?
		`)}\n\n${info}`;
	...
	collector
		...
		.then(() => {
			intr.guild.members
				...
				.then(() => {
					intr.guild.members
						...
						.then(async () => {
							...
							intr.editReply({
								content: `${oneLine(e`
								{check} Successfully ${bold`softbanned ${target.tag}`}
						...
						.catch(() => {
							intr.editReply({
								content: stripIndent(e`
									{cross} Failed to softban ${target.tag} (${target.id})
									
									{warning} The user did ${bold("not get unbanned")}
				...
				.catch(() => {
					intr.editReply({
						content: e`{cross} Failed to softban ${target.tag} (${target.id})\n\n${info}`,
						components: []
					});
				});
		})
