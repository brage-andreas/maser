...
const sizeChoices = [16, 32, 64, 128, 256, 512, 1024, 2048, 4096].map(
	(size) => ({
		name: `${size}px`,
		value: size
	})
...
async function execute(intr: ChatInputCommandInteraction<"cached">) {
	...
	const getURL = (target: GuildMember | User, format: string) => {
		...
		const defaultBase = `https://cdn.discordapp.com/embed/avatars/${
	...
	const getAvatarLinks = (target: GuildMember | User, altText: string) => {
		const arr = ["png", "jpg", "webp"].map(
			(format) =>
				`[${format}](${getURL(user, format)} "${altText.replace(
					"{format}",
					format
				)}")`
		);
		...
		if (target.displayAvatarURL().endsWith(".gif"))
			arr.splice(
				0,
				0,
				`[${"gif"}](${getURL(user, "gif")} "${altText.replace(
					"{format}",
					"gif"
				)}")`
			);
	...
	const guildAvatarLinks = member
		? getAvatarLinks(member, "Guild avatar in {format} format")
		: null;
	...
	const description = `**User**: ${user}\n${
		...
		hasGuildAvatar
			? `**Guild avatar**: ${guildAvatarLinks!.join(", ")}\n`
			: ""
	...
	if (hasGuildAvatar) {
		const collector = buttonManager
			.setMessage(msg)
			.createCollector({ time: "30s" });
		...
		collector.on("collect", async (interaction) => {
			...
			//
				...
				embed.image = {
					url: (member ?? user).displayAvatarURL({ size })
				};
